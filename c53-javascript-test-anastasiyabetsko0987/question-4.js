...
const recommendedNextVideo = (users = [], videos = [], userId = null) => {
  ...
  const favoritedVideoIds = user.favoritedVideoIds;

  // create a list of videos that the user has not seen yet!
  const notSeenVideos = videos.filter(
    (video) => !favoritedVideoIds.includes(video.id)
  );

  // find the video with the most tags in common with the favoriteVideo tags
  let mostTags = 0;
  let recommended = undefined;
  const favoritedVideos = videos.filter((video) =>
  ...
  notSeenVideos.forEach((video) => {
    ...
    if (commonTagsCount > mostTags) {
      mostTags = commonTagsCount;
      recommended = video;
    }
  });
  return recommended ? recommended.id : undefined;
};
